...
def build_report():
    ...
    architecture = " -> ".join([*(str(width) for width in hidden_layers), "1"])
    activation = {"relu": "ReLU", "gelu": "GELU", "leaky_relu": "LeakyReLU", "silu": "SiLU"}[params["activation"]]
    normalization = {"none": "Ninguna", "layer": "LayerNorm", "batch": "BatchNorm"}[params["normalization"]]
    optimizer = {"adam": "Adam", "adamw": "AdamW"}[params["optimizer"]]
    ...
    complete_trials = trials.loc[trials.state == "COMPLETE"]
    optuna_best_id = f"optuna_{int(complete_trials.loc[complete_trials.value.idxmin(), 'number']):03d}"
    ...
    doc.add_paragraph(
        f"Se desarrolló un MLP para regresión sobre {eda['rows']:,} viviendas, {eda['columns'] - 2} variables predictoras y la columna Id. "
    ...
    doc.add_paragraph(
        ...
        f"numéricos, {eda['categorical_features']} categóricos y SalePrice. No hay filas ni Id duplicados. MSSubClass "
    ...
    doc.add_paragraph(
        ...
        "borrar observaciones posiblemente válidas, el pipeline ganador aprendió límites en los percentiles 1 y 99 dentro de "
        "cada fold. Esta decisión reduce la influencia de extremos sobre gradientes sin usar información de validación."
    )
    ...
    doc.add_paragraph(
        ...
        "(GarageCars/GarageArea o 1stFlrSF/TotalBsmtSF) no obliga a eliminarlas en un MLP regularizado, pero sí aumenta el riesgo de sobreajuste."
    )
    ...
    for text in [
        "Categóricas: Missing explícito, one-hot, categorías infrecuentes agrupadas con min_frequency=5 y tolerancia a niveles no vistos.",
        "Numéricas: mediana e indicador de ausencia, clipping 1/99, log1p solo para columnas no negativas con asimetría absoluta mayor a 0.75 y RobustScaler.",
        "Ingeniería de variables: superficie total, baños equivalentes, porches, antigüedad al vender, tiempo desde remodelación e interacciones calidad-superficie.",
    ...
    doc.add_heading("2.1 Separación y control de leakage", level=2)
    doc.add_paragraph(
        ...
        "en una partición nueva de cinco folds (seed 31415). La mediana de épocas de esa CV se fijó antes de abrir el holdout; "
        "el modelo de validación se reentrenó desde cero durante ese número fijo de épocas. Imputadores, umbrales de asimetría, "
        "percentiles, escaladores, agrupación de categorías y one-hot se ajustaron exclusivamente en el train de cada fold."
    )
    doc.add_page_break()
    doc.add_heading("2.2 Arquitectura y entrenamiento", level=2)
    cfg_rows = [
        ("Arquitectura", architecture), ("Activación", activation),
        ("Dropout", f"{params['dropout']:.4f}"), ("Normalización", normalization),
        ("Optimizador", optimizer), ("Learning rate", f"{params['learning_rate']:.7f}"),
        ("Weight decay", f"{params['weight_decay']:.7f}"), ("Batch size", str(params['batch_size'])),
        ("Loss", "MSE sobre SalePrice estandarizado"), ("Gradient clipping", "Norma máxima 5.0"),
        ("Scheduler", "ReduceLROnPlateau, factor 0.5"), ("Early stopping", "Paciencia 50"),
    ]
    add_table(doc, ["Componente", "Configuración final"], cfg_rows, [3300, 6060], font_size=9.5)
    doc.add_paragraph(
    ...
    doc.add_heading("2.3 Búsqueda de hiperparámetros", level=2)
    doc.add_paragraph(
        f"Optuna con TPE exploró {len(trials)} trials y podó {int((trials.state == 'PRUNED').sum())} mediante MedianPruner. Se variaron "
        "profundidad/ancho, ReLU/GELU/LeakyReLU/SiLU, dropout, normalización, Adam/AdamW, learning rate, weight decay, batch size, "
        "target original/log1p, Standard/RobustScaler, clipping, umbral de asimetría, frecuencia mínima e ingeniería de features. "
    ...
    major_ids = ["baseline_raw", "baseline_log", "feature_engineered", "deep_low_regularization",
                 "robust_regularized", optuna_best_id, "confirm_optuna_winner",
    ...
    labels = {
        ...
        optuna_best_id: "Ganador Optuna 3-fold", "confirm_optuna_winner": "Ganador confirmado 5-fold",
    ...
    cite = doc.add_paragraph("Fuente: experiments/results.csv, generado automáticamente por ejecuciones reales.")
    ...
    doc.add_paragraph(
        ...
        f"La red profunda sin regularización alcanzó RMSE train de {results.loc[results.experiment_id == 'deep_low_regularization', 'train_rmse'].iloc[0]:,.0f} "
        f"pero CV de {results.loc[results.experiment_id == 'deep_low_regularization', 'cv_rmse_mean'].iloc[0]:,.0f}, evidencia de sobreajuste. "
        f"El baseline log1p obtuvo {results.loc[results.experiment_id == 'baseline_log', 'cv_rmse_mean'].iloc[0]:,.0f}: simetrizar el target "
        "no alineó la loss con la penalización absoluta del RMSE oficial. "
    ...
    add_figure(doc, "training_baseline_raw.png", "Figura 5. Loss y RMSE de entrenamiento/validación del MLP baseline.")
    add_figure(doc, "training_final_cv.png", "Figura 6. Loss y RMSE de entrenamiento/validación del modelo ganador en CV.")
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        ...
        "indica subestimación; en el segmento bajo el sesgo negativo indica sobreestimación. Este patrón es coherente con regresión "
    ...
    doc.add_paragraph(
        ...
        "pero sí heterocedasticidad: la amplitud de los residuos aumenta con la predicción."
    )
    doc.add_page_break()
    ...
    for text in [
        ...
        f"La búsqueda de {len(trials)} trials es seria pero no exhaustiva; más seeds y repeated CV reducirían incertidumbre a mayor costo computacional.",
    ]:
        add_bullet(doc, text)

    doc.add_heading("5. Conclusiones", level=1)
    doc.add_paragraph(
        f"El MLP final usa capas {'-'.join(str(width) for width in hidden_layers)}, {activation}, dropout {params['dropout']:.3f}, "
        f"{optimizer}, learning rate {params['learning_rate']:.7f}, weight decay {params['weight_decay']:.7f} y batch {params['batch_size']}. "
        "Con preprocessing reproducible e "
    ...
    doc.add_paragraph(
        ...
        "regularizar redes profundas y confirmar los mejores trials con una partición distinta. Como mejoras futuras convendría "
        "repetir CV con más seeds, explorar embeddings categóricos, ensembles de MLPs con seeds distintas y una loss ponderada/calibración "
        "que reduzca la subestimación de propiedades caras, siempre validando el RMSE original."
    )
